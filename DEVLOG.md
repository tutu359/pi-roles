...
- 菜单内改动**一律不弹通知**（选角色、切模式、切拦截）
- **Esc 退出菜单时统一报一次结果**：`已启用 tutor · append · 自动拦截开`；未选角色时报「未启用角色」
- 只有本菜单会话内**真的改动过**才报（`menuChanged` 标志），开菜单看一眼就 Esc 不会打扰
- 反馈职责归位：**菜单负责中途反馈、状态栏负责常驻显示、通知只报异常与最终结果**
